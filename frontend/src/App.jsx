...
export default function App() {
  ...
  // ✅ Fetch all listings on first render if not logged in
  useEffect(() => {
    if (view === "dashboard" && dashboardTab === "all") {
      fetchListings();
    }
  }, []);
            ...
            <div className="flex space-x-4">
              <button
                className={`border border-black px-4 py-2 rounded font-semibold ${
                  dashboardTab === "all" ? "bg-black text-white" : ""
                }`}
                onClick={() => setDashboardTab("all")}
              >
                All Listings
              ...
              {user && (
                <button
                  className={`border border-black px-4 py-2 rounded font-semibold ${
                    dashboardTab === "my" ? "bg-black text-white" : ""
                  }`}
                  onClick={() => setDashboardTab("my")}
                >
                  My Listings
